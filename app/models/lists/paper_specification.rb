...
class Lists::PaperSpecification < ActiveRecord::Base
  ...
  scope :pricelist, -> {lambda { |order_type| where("order_type_id=#{order_type.id} OR order_type_id=1").where(:in_stock => true ).joins(:paper_size).order('lists_paper_specifications.price') }}
